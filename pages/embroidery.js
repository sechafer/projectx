...
const Embroidery = ({ products }) => {
  ...
  return (
    <div>
    <head><title>Embroidery- The House Craft</title></head>
      <section className="text-gray-600 body-font">
        <div className="container md:ml-32 px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4">
            {Object.keys(products).length===0 && <p className='md:mx-96'>Sorry!! .All Embroidery are out of stock  .</p> }
...
export async function getServerSideProps(context) {
  ...
  let products = data.products.filter(x => x.category == "Embroidery");
  let variant = {};
